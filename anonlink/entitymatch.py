...
def cffi_filter_similarity_k(filters1, filters2, k, threshold):
    ...
    # There's no sense in having k > length_f2 or k < 1. Also, k is
    # passed to ffi.new(...) below, so we need to protect against an
    # out-of-memory DoS if k is of untrustworthy origin.
    k = max(1, min(k, length_f2))  # ensure 1 <= k <= length_f2
